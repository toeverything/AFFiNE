...
export function isFarEnough(a: IPoint, b: IPoint, thresholdSq?: number) {
  const dx = a.x - b.x;
  const dy = a.y - b.y;
  ...
  const platformThresholdSq = IS_IPAD || IS_IOS ? 64 : 4;
  return (
    Math.pow(dx, 2) + Math.pow(dy, 2) > (thresholdSq ?? platformThresholdSq)
  );
}
