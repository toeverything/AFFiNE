...
Коллаборация (§6.6) — `src/collab`: pointer/viewport в awareness (`wbCollab`, throttle 40ms), follow по аватару, пульс «посмотри сюда» (5s), пины комментариев на gfx-блоках, `commentId` на карточке канбана, lock второго live-editor. Named versions — подпись снимка в истории документа (не git). Undo остаётся per-client; у sketch — свой `Y.UndoManager`. Флаг `enable_whiteboard_collab` по умолчанию выключен.

Инфраструктура продукта (§6.8) не дублирует `@affine/server`: workspace = tenant, RBAC через `store.readonly` / `DocRole` (`canEditBoardWidgets`) и soft-lock `lockedBySelf`. Снимки chart/sketch пишутся в существующий blob engine; старый `snapshotBlobId` сбрасывается, серверный GC (30 дней) подбирает нессылаемые блобы — `BlobEngine.delete()` не вызываем. Телеметрия HUD: `echarts_init_ms`, `snapshot_age_s`, dropped frames, `board_object_count`, `live_collaborators`, Yjs apply, размер WS. Форматы: PNG/SVG виджета и доски, `.excalidraw`, загрузка CSV в chart, Mermaid → inline table; draw.io XML и Miro CSV — парсеры без полного импортёра.
