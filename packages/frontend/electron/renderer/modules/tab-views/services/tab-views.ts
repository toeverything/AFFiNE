import {
  ...
  type RouteModulePath,
} from '@affine/core/modules/navigation/utils';
...
import { type GlobalState, Service } from '@toeverything/infra';
...
export class DesktopTabViewsService extends Service {
  ...
  get tabViewsMeta() {
    return tabViewsMetaSchema.parse(this.globalState.get(TabViewsMetaKey));
  }

  set tabViewsMeta(meta: TabViewsMetaSchema) {
    this.globalState.set(TabViewsMetaKey, meta);
  }

  tabViewsMeta$ = this.globalState.watch(TabViewsMetaKey).pipe(
    map(meta => {
      return tabViewsMetaSchema.parse(meta);
    })
  );
