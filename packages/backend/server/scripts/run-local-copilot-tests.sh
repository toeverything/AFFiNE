...
source "$SERVER_ROOT/scripts/setup-local-test-env.sh"
...
export REDIS_SERVER_DATABASE=${REDIS_SERVER_DATABASE:-0}
export NODE_ENV=${NODE_ENV:-test}

for spec in "$SERVER_ROOT"/src/__tests__/copilot/*.spec.ts; do
  "$REDIS_PREFIX/bin/redis-cli" -h "$REDIS_SERVER_HOST" -p 6379 -n "$REDIS_SERVER_DATABASE" FLUSHDB >/dev/null
