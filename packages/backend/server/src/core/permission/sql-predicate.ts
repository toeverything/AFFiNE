  ...
  docReadableByNewTables(input: {
    ...
    return {
      sql: [
        ...
        `AND (`,
        `(wm.id IS NOT NULL AND dg.role = ANY(?))`,
        `OR (wm.id IS NULL AND wap.sharing_enabled AND dg.role = ANY(?))`,
        `OR wm.role = ANY(?)`,
        `OR (wm.id IS NOT NULL AND dg.principal_id IS NULL AND COALESCE(dap.member_default_role, wap.member_default_doc_role) = ANY(?))`,
        `OR (wap.sharing_enabled AND dap.visibility = 'public' AND dap.public_role = ANY(?))`,
        `))`,
      ].join(' '),
      params: [
        input.userId,
