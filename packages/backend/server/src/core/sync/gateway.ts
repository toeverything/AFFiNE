...
interface LeaveAwarenessMessage {
  spaceType: SpaceType;
  spaceId: string;
  docId: string;
}
...
interface PreSyncSpaceDocsMessage {
  spaceType: SpaceType;
  spaceId: string;
  timestamp?: number;
}

interface LoadAwarenessesMessage {
  spaceType: SpaceType;
  spaceId: string;
  docId: string;
}
  ...
  implements OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async onLoadSpaceDoc(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    { spaceType, spaceId, docId, stateVector }: LoadSpaceDocMessage
  ): Promise<
  ...
  @SubscribeMessage('space:doc-updates')
  async onReceiveDocUpdates(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    message: BroadcastDocUpdateMessage
  ): Promise<EventResponse<{ accepted: true; timestamp?: number }>> {
    const { spaceType, spaceId, docId, updates } = message;
    const adapter = this.selectAdapter(client, spaceType);
    await adapter.assertAccessible(spaceId, client.id, Permission.Write);
    ...
    // so
    client.to(adapter.room(spaceId)).emit('space:doc-updates', message);
  ...
  @SubscribeMessage('space:pre-sync-docs')
  async onPreSyncSpaceDocs(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    { spaceType, spaceId, timestamp }: PreSyncSpaceDocsMessage
  ...
  async onLeaveAwareness(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    { spaceType, spaceId, docId }: LeaveAwarenessMessage
  ) {
  ...
  async onAwarenessInit(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    { spaceType, spaceId, docId }: LoadAwarenessesMessage
  ) {
    const adapter = this.selectAdapter(client, spaceType);
    ...
    client
      ...
      .emit('space:collect-awarenesses');
  ...
  @SubscribeMessage('space:awareness-update')
  async onAwarenessUpdate(
    @ConnectedSocket() client: Socket,
    @MessageBody() message: AwarenessUpdateMessage
  ) {
    ...
    client
      .to(adapter.room(spaceId, roomType))
      .emit('space:awareness-update', message);
  ...
  async loadDocStats(
    ...
    return this.onPreSyncSpaceDocs(client, {
      spaceType: SpaceType.Workspace,
      spaceId: workspaceId,
      timestamp,
    });
  ...
  async handleInitAwareness(
    ...
    return this.onAwarenessInit(client, {
      spaceType: SpaceType.Workspace,
      spaceId: workspaceId,
      docId: workspaceId,
    });
  ...
  async handleHelpGatheringAwareness(
    ...
    return this.onAwarenessUpdate(client, {
      spaceType: SpaceType.Workspace,
      spaceId: workspaceId,
      docId: workspaceId,
      awarenessUpdate,
