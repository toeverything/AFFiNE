...
use super::{
  ...
  ChargeSubject, chargeable_invitation_statuses, invalidate_storage_usage, load_decision_time,
  ...
  pub async fn reserve_storage_quota_v1(
    ...
    loop {
      ...
      let existing = if resource == StorageResource::Blob {
        sqlx::query(
          "SELECT size,mime,status::text AS status,reservation_id,reservation_expires_at,deleted_at FROM blobs WHERE \
          ...
          let deleted = if resource == StorageResource::Blob {
            ...
            sqlx::query(
              "UPDATE comment_attachments SET deleted_at=COALESCE(deleted_at,clock_timestamp()) WHERE workspace_id=$1 \
               AND doc_id=$2 AND key=$3 AND status='completed'",
            )
            .bind(&input.workspace_id)
            .bind(input.doc_id.as_deref())
            .bind(&input.key)
      ...
      invalidate_storage_usage(self, &input.workspace_id, &subject.owner_id).await;
      self
        .permission_telemetry
