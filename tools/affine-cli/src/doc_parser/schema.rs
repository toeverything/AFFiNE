...
// `affine:latex` block (math equation). `prop:latex` is the TeX source; the rest are the
// GfxBlock defaults from `LatexBlockSchema` so the block also behaves on the edgeless surface.
pub(super) const PROP_LATEX: &str = "prop:latex";
pub(super) const PROP_SCALE: &str = "prop:scale";
pub(super) const PROP_ROTATE: &str = "prop:rotate";
pub(super) const PROP_LOCKED_BY_SELF: &str = "prop:lockedBySelf";
