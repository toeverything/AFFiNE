...
use super::{
    super::{
        ...
        schema::{
            PROP_BACKGROUND, PROP_DISPLAY_MODE, PROP_ELEMENTS, PROP_HIDDEN, PROP_INDEX, PROP_XYWH, SURFACE_FLAVOUR,
        },
    },
    builder::{
...
mod tests {
    ...
    #[test]
    fn test_markdown_math_block_and_inline_shape() {
        use y_octo::{TextDeltaOp, TextInsert};

        let markdown = "Euler $e^{i\\pi}+1=0$ rocks.\n\n$$\\int_0^1 x^2 dx$$";
        let doc_id = "math-shape";
        let bin = build_full_doc("Math", markdown, doc_id).expect("create doc");

        let mut doc = DocOptions::new().with_guid(doc_id.to_string()).build();
        doc.apply_update_from_binary_v1(&bin).expect("apply update");
        let blocks_map = doc.get_map("blocks").expect("blocks map");

        let mut latex_block = None;
        let mut paragraph = None;
        for (_, value) in blocks_map.iter() {
            let Some(block_map) = value.to_map() else { continue };
            match get_string(&block_map, "sys:flavour").as_deref() {
                Some("affine:latex") => latex_block = Some(block_map),
                Some("affine:paragraph") => paragraph = Some(block_map),
                _ => {}
            }
        }

        // Block equation: `prop:latex` holds the TeX source, plus the GfxBlock defaults so it also
        // behaves on the edgeless surface (matches LatexBlockSchema / `add_latex_block`).
        let latex_block = latex_block.expect("affine:latex block present");
        assert_eq!(
            get_string(&latex_block, "prop:latex").as_deref(),
            Some("\\int_0^1 x^2 dx")
        );
        assert_eq!(get_string(&latex_block, "prop:xywh").as_deref(), Some("[0,0,16,16]"));
        assert_eq!(get_string(&latex_block, "prop:index").as_deref(), Some("a0"));
        assert!(latex_block.get("prop:scale").is_some(), "prop:scale set");
        assert!(latex_block.get("prop:rotate").is_some(), "prop:rotate set");
        assert!(latex_block.get("prop:lockedBySelf").is_some(), "prop:lockedBySelf set");

        // Inline equation: a single-space insert carrying a string `latex` attribute. This is the
        // exact shape AFFiNE renders (`inline-spec.ts`: `typeof delta.attributes?.latex === 'string'`).
        let paragraph = paragraph.expect("paragraph present");
        let text = paragraph.get("prop:text").and_then(|v| v.to_text()).expect("prop:text");
        let inline_latex = text.to_delta().into_iter().find_map(|op| match op {
            TextDeltaOp::Insert {
                insert: TextInsert::Text(s),
                format: Some(attrs),
            } => attrs.get("latex").map(|latex| (s, latex.clone())),
            _ => None,
        });
        let (insert, latex_attr) = inline_latex.expect("inline latex op present");
        assert_eq!(insert, " ", "inline latex insert is a single space");
        assert_eq!(latex_attr, Any::String("e^{i\\pi}+1=0".to_string()));
    }
