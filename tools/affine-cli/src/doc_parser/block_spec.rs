...
use super::{
    ...
    schema::{
        ...
        PROP_URL, PROP_VIDEO_ID, PROP_WIDTH, SYS_FLAVOUR, table_cell_text_key,
    },
...
impl BlockFlavour {
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            BlockFlavour::Latex => "affine:latex",
        }
    }
    ...
    pub fn from_str(value: &str) -> Option<Self> {
        match value {
            ...
            "affine:latex" => Some(BlockFlavour::Latex),
            _ => None,
        }
    }
}
...
impl BlockSpec {
    ...
    pub(super) fn from_block_map_with_flavour(block: &Map, flavour: BlockFlavour) -> Self {
        ...
        if flavour == BlockFlavour::Latex {
            // The TeX source lives in `prop:latex`; we carry it as a plain-text delta in `text`
            // (the same way `affine:code` keeps its body), so block equality/diffing reuse `text`.
            let latex = get_string(block, PROP_LATEX).unwrap_or_default();
            let text = if latex.is_empty() {
                Vec::new()
            } else {
                vec![TextDeltaOp::Insert {
                    insert: TextInsert::Text(latex),
                    format: None,
                }]
            };
            return BlockSpec {
