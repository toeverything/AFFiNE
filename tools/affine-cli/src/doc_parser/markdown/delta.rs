...
fn convert_delta_ops(ops: &[DeltaOp], options: &DeltaToMdOptions) -> Rc<RefCell<Node>> {
    ...
    for index in 0..ops.len() {
        ...
        // inline styles first so the equation stands on its own.
        if let Some(latex) = op
            .attributes
            .get(InlineStyle::Latex.key())
            .and_then(any_as_string)
            .filter(|latex| !latex.is_empty())
        {
            apply_inline_attributes(&mut el, &TextAttributes::new(), None, &mut active_inline, options);
            Node::append(&el, Node::new_text(&format!("${latex}$")));
            ...
            DeltaInsert::Text(text) => {
                ...
                if has_block_level_attribute(&op.attributes) {
                    ...
                    for (line_index, segment) in lines.iter().enumerate() {
                        ...
                        // Inside a code span `$` is inert (and a backslash would corrupt the code); everywhere
                        // else escape math-significant dollars so literal text survives re-parsing.
