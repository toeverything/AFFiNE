...
fn markdown_options() -> Options {
    ...
    // `$…$` → InlineMath (inline equation), `$$…$$` → DisplayMath (block equation). The dollar
    // rules avoid currency false-positives ("$5 and $10" stays literal text).
    | Options::ENABLE_MATH
}

/// Build the `latex` attribute for an inline math op: a single-space insert whose `latex`
/// attribute carries the TeX source (BlockSuite's `{ insert: ' ', attributes: { latex } }`).
fn latex_attrs(content: &str) -> Option<TextAttributes> {
    let mut attrs = TextAttributes::new();
    attrs.insert(InlineStyle::Latex.key().into(), Any::String(content.to_string()));
    Some(attrs)
}
...
fn parse_markdown_inner(markdown: &str) -> Result<MarkdownDocument, ParseError> {
    ...
    for event in parser {
        ...
        if let Some(state) = table_state.as_mut() {
            match &event {
                ...
                Event::InlineMath(content) | Event::DisplayMath(content) => {
                    // Table cells are plain strings, so preserve the equation as literal `$…$` /
                    // `$$…$$` markdown rather than dropping it.
                    let fragment = if matches!(&event, Event::DisplayMath(_)) {
                        format!("$${}$$", content.trim())
                    } else {
                        format!("${content}$")
                    };
                    if let Some(image) = state.pending_image.as_mut() {
                        image.caption.push_str(&fragment);
                    } else {
                        state.push_text(&fragment);
                    }
                    table_handled = true;
        ...
        match event {
          ...
          // A paragraph fully consumed by a standalone `$$…$$` (or emptied by `$$$$`) must not
          // leave a stray empty paragraph block behind.
          && !block.is_empty()
                {
                    attach_block(block.finish(), &mut list_items, &mut blocks);
                }
            ...
            Event::InlineMath(content) => {
                // `$…$` → inline math: a single-space insert carrying the `latex` attribute.
                if pending_bookmark.is_some() {
                    pending_bookmark = None;
                }
                if let Some(image) = pending_image.as_mut() {
                    image.caption.push_str(&format!("${content}$"));
                } else {
                    let attrs = latex_attrs(&content);
                    if let Some(block) = active.as_mut() {
                        block.push_text(" ", attrs);
                    } else if let Some(item) = list_items.last_mut() {
                        item.push_text(" ", attrs);
                    }
                }
            }
            Event::DisplayMath(content) => {
                // `$$…$$` → a block equation (`affine:latex`) when it stands alone in a paragraph;
                // otherwise (mid-paragraph, list, blockquote) fall back to an inline equation.
                ...
                let standalone = !in_blockquote
                    && list_items.is_empty()
                    && pending_image.is_none()
                    && active.as_ref().is_none_or(|b| {
                        b.is_empty()
                            && b.flavour == BlockFlavour::Paragraph
                            && matches!(b.block_type, Some(BlockType::Text) | None)
                    });
                if standalone {
                    let mut draft = BlockDraft::new(BlockFlavour::Latex, None);
                    draft.push_text(content, None);
                    attach_block(draft.finish(), &mut list_items, &mut blocks);
                    // Re-open an empty paragraph so inline content that follows the equation in the SAME
                    // paragraph (`$$…$$ trailing text`) still has a home; End(Paragraph) drops it if it
                    // stays empty.
                    active = Some(BlockDraft::new(BlockFlavour::Paragraph, Some(BlockType::Text)));
...
fn validate_markdown_inner(markdown: &str) -> Result<(), ParseError> {
    ...
    for event in parser {
        match event {
            ...
            // `$…$` / `$$…$$` are supported: inline equations and `affine:latex` blocks.
            _ => {}
        }
    }
