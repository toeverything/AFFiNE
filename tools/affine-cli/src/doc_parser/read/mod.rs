...
const KNOWN_UNSUPPORTED_MARKDOWN_FLAVOURS: [&str; 9] = [
    "affine:attachment",
    "affine:callout",
    "affine:note",
...
fn is_edgeless_markdown_flavour(flavour: &str) -> bool {
    matches!(flavour, "affine:surface" | "affine:frame" | "affine:surface-ref")
        || flavour.starts_with("affine:edgeless-")
}
    ...
    while let Some((parent_block_id, block_id)) = walker.next() {
        ...
        if matches!(flavour.as_str(), "affine:paragraph" | "affine:list" | "affine:code") {
            if let Some(text) = block.get("prop:text").and_then(|value| value.to_text()) {
                let database_name =
                    if flavour == "affine:paragraph" && parent_flavour.as_deref() == Some("affine:database") {
