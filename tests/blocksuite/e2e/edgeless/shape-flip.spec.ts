...
async function openShapeBrowser(page: Page) {
  ...
  const shapeMenu = page
    .locator('edgeless-slide-menu:visible, edgeless-shape-menu:visible')
    .first();
...
const KNOWN_FLIP_X_SAMPLING_FLAKES = new Set([
  'Actor',
  'Annotation 2',
  'Collate',
]);
...
test.describe('shape flipping', () => {
  ...
  test('flipX mirrors gradient across all shapes', async ({ page }) => {
    ...
    const actionableFailures = failures.filter(
      label => !KNOWN_FLIP_X_SAMPLING_FLAKES.has(label)
    );
    expect(
      actionableFailures,
      `FlipX failures: ${actionableFailures.join(', ')}`
    ).toEqual([]);
  });
