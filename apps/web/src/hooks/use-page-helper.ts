...
import { PageMeta, useGlobalStateApi } from '@affine/store';
...
export const usePageHelper = (): EditorHandlers => {
  ...
  return {
    createPage: (
      { pageId = uuidv4().replaceAll('-', ''), title = '' } = {},
      targetWorkspace?: WorkspaceUnit | null
    ) => {
      return new Promise(resolve => {
        targetWorkspace = targetWorkspace ?? currentWorkspace;
        if (!targetWorkspace) {
          return resolve(null);
        }
        targetWorkspace.blocksuiteWorkspace?.signals.pageAdded.once(
          addedPageId => {
            targetWorkspace?.blocksuiteWorkspace?.setPageMeta(addedPageId, {
              title,
            });
            if (targetWorkspace) {
              api.setState({
                dataCenterPageList:
                  (targetWorkspace.blocksuiteWorkspace?.meta
        ...
        targetWorkspace.blocksuiteWorkspace?.createPage(pageId);
      });
    },
